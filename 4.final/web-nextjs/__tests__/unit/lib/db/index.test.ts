...
const { getPool, query } = require('@/infrastructure/database/connection');

describe('Database Connection (lib/db/index.ts)', () => {
  beforeEach(() => {
    resetDbMocks();
  ...
  describe('environment configuration', () => {
    test('should use environment variables for database config', () => {
      // The pool should be configured with environment variables
      const pool = getPool();
      expect(pool).toBeDefined();
      // Configuration is tested implicitly through pool creation
    });
  });
});
