...
class APIClient {
  ...
  async searchPrendas(termino) {
    return this.request(`/prendas/buscar/${encodeURIComponent(termino)}`);
  }
  ...
  async searchClientes(termino) {
    return this.request(`/clientes/buscar/${encodeURIComponent(termino)}`);
  }

  async getClientesEstadisticas() {
    return this.request('/clientes/estadisticas');
  }
