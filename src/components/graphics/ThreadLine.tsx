 ...
 */
function getPathForMovement(movement: ThreadMovement, width: number, height: number): string {
  const w = width;
  const h = height;
  const midY = h / 2;
  ...
  switch (movement) {
    case 'air':
      ...
      return `M 0 ${h * 0.7} Q ${w * 0.25} ${h * 0.3} ${w * 0.5} ${midY} T ${w} ${h * 0.2}`;

    case 'water':
      // Smooth, wave-like, horizontal flow
      return `M 0 ${midY} Q ${w * 0.25} ${h * 0.3} ${
        w * 0.5
      } ${midY} T ${w} ${midY} Q ${w * 0.75} ${h * 0.7} ${w * 0.5} ${midY} T 0 ${midY}`;

    case 'fire':
      // Dynamic, upward flickering - sharp peaks
      return `M 0 ${h * 0.8} L ${w * 0.2} ${h * 0.4} L ${w * 0.3} ${
        h * 0.6
      } L ${w * 0.5} ${h * 0.2} L ${w * 0.7} ${h * 0.5} L ${w * 0.8} ${h * 0.3} L ${w} ${h * 0.1}`;
    ...
    default:
      ...
      return `M 0 ${h * 0.5} C ${w * 0.2} ${h * 0.2} ${w * 0.4} ${h * 0.8} ${
        w * 0.6
      } ${h * 0.3} C ${w * 0.8} ${h * 0.7} ${w * 0.9} ${h * 0.1} ${w} ${h * 0.5}`;
  }
}
