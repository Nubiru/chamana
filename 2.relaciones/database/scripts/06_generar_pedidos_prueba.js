...
async function generarPedidosPrueba() {
  ...
  try {
    ...
    const clientesResult = await pool.query('SELECT id FROM clientes WHERE activo = true LIMIT 10');
    ...
    if (clientesResult.rows.length === 0 || prendasResult.rows.length === 0) {
      throw new Error('No hay clientes o prendas disponibles para generar pedidos');
    }
    ...
    for (let i = 0; i < orderSpecs.length; i++) {
      ...
      try {
        ...
        console.log(`📦 Pedido ${orderNum}/${orderSpecs.length}: ${spec.purpose}`);
        ...
        // Generate order date
        const fechaPedido = spec.daysAgo === 0 ? new Date() : randomPastDate(spec.daysAgo);
        ...
        for (const item of orderItems) {
          await executeQuery(
            ...
            VALUES ($1, $2, $3, $4, $5)
          `,
            [pedidoId, item.prenda_id, item.cantidad, item.precio_unitario, item.subtotal]
          );

          console.log(`      • ${item.nombre} x${item.cantidad} = $${item.subtotal.toFixed(0)}`);
        }
        ...
        if (spec.estado === 'completado') {
          ...
          console.log(`   ✅ Pedido #${pedidoId} completado, stock actualizado`);
          ...
          console.log(`   🚫 Pedido #${pedidoId} cancelado (sin cambio de stock)`);
        } else {
    ...
    console.log('   • Movimientos de inventario registrados para todos los completados\n');
  } catch (error) {
